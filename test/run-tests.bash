...
if [[ $(query "SELECT to_regclass('tiger_data.state_all') IS NOT NULL" | tr -d '[:space:]') != "t" ]]; then
  ...
  if [[ $(query "SELECT count(*) > 0 FROM api.load_log WHERE state = 'DC'" | tr -d '[:space:]') != "t" ]]; then
    ...
    query "DELETE FROM ONLY tiger.zip_state WHERE stusps = 'AL';
    ...
    # Now make Alabama the prefix's majority claimant. The parsed state still
    # wins because it claims the zip: 83414 is Alta, WY, under an Idaho prefix.
    query "INSERT INTO tiger.zip_state (zip, stusps, statefp)
           SELECT (20100 + g)::text, 'AL', '01' FROM generate_series(1, 40) AS g" >/dev/null

    expect_true "a parsed state that claims the zip outranks the prefix census" \
      "SELECT count(*) = 1 AND max(state) = 'DC' AND max(rating) <= 20
       FROM api.geocode('1731 New Hampshire Ave NW, Washington, DC 20009', 1)"

    query "DELETE FROM ONLY tiger.zip_state WHERE stusps = 'AL'" >/dev/null
