-- The baseline: the three ranking helpers and geocode_address exactly as
-- postgis_tiger_geocoder 2025.2 ships them, install-time placeholders resolved
-- as this image resolves them (public). ab.sh and diff.sh install this, measure,
-- then install sql/30-performance-fixes.sql and measure again.
...
CREATE OR REPLACE FUNCTION tiger.greatest_hn(fromhn character varying, tohn character varying)
 RETURNS integer
 LANGUAGE sql
 IMMUTABLE PARALLEL SAFE COST 200
  ...
  -- In pprint_addy we changed to put it in front if it is a is_hw type
    stmt := 'SELECT DISTINCT ON (sub.predirabrv,sub.fename,COALESCE(sub.suftypabrv, sub.pretypabrv) ,sub.sufdirabrv,coalesce(p.name,cs.name,zip.city,co.name),s.stusps,sub.zip)'
         ...
         || CASE WHEN zip_info.exact
                 ...
                 ELSE '    AND ( public.soundex($2) = public.soundex(a.name)  OR ( (length($2) > 15 or (length($2) > 7 AND a.prequalabr > '''') ) AND lower(a.fullname) LIKE lower(substring($2,1,15)) || ''%'' ) OR  tiger.numeric_streets_equal($2, a.name) ) '
            END
         || '  ORDER BY 11'
         || '  LIMIT 200'
         || '    ) AS sub'
