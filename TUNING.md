...
| `max_parallel_workers` | physical cores | SMT siblings share execution units; counting them inflates the pools past what the box can run |
| `max_worker_processes` | physical cores, at least 8 | The ceiling on background workers of every kind, not a pool; below PostgreSQL's default an extension that needs one fails to start |
